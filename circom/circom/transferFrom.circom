...
template ConfTransferFrom(NBITS) {
    // -------------------- Public inputs --------------------
    // Sender and receiver public keys
    signal input y[2];        // spender pubkey
    signal input yR[2];       // receiver/to pubkey
    signal input yF[2];       // from pubkey

    // Current allowance ciphertext for spender
    signal input CL[2];       // current allowance of spender
    signal input CR[2];       // current allowance of spender

    signal input CS[2];       // C_send
    signal input D[2];        // D_send
    signal input CRe[2];      // C_receive
    signal input CFr[2];      // C_from
    signal input counter;     // not used in constraints, counter of the spender
    ...
    component chkYF = BabyCheck();  chkYF.x <== yF[0];          chkYF.y <== yF[1];
    ...
    component chkCFr = BabyCheck();  chkCFr.x <== CFr[0];       chkCFr.y <== CFr[1];
    ...
    // [r]ȳ
    component mul_r_yF = EscalarMulAny(NBITS);
    mul_r_yF.p[0] <== yF[0];
    mul_r_yF.p[1] <== yF[1];
    ...
    component ad_D_CFr = BabyAdd();
    ad_D_CFr.x1 <== mul_sAmount_G.out[0];
    ad_D_CFr.y1 <== mul_sAmount_G.out[1];
    ad_D_CFr.x2 <== mul_r_yF.out[0];
    ad_D_CFr.y2 <== mul_r_yF.out[1];

    // Enforce equality with provided ciphertexts
    CS[0] === ad_D_C.xout;   CS[1] === ad_D_C.yout;
    CRe[0] === ad_D_CRe.xout; CRe[1] === ad_D_CRe.yout;
    CFr[0] === ad_D_CFr.xout; CFr[1] === ad_D_CFr.yout;
...
component main{public [MAX, y, yR, yF, CL, CR, CS, CRe, CFr, D, counter]} = ConfTransferFrom(252);
